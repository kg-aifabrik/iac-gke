...
| 07 | *(drain survival)* | Spread + PDB + gateway health checks | A node hosting a replica **drains with zero failed requests** through the gateway |
| 08 | *(rolling deploy)* | Readiness-gated rollouts | A rolling restart completes with **zero failed requests** |
...
`restore_plan_name`); override via the same-named environment variables. The
external test uses `curl --resolve` to the gateway IP, so public **DNS
propagation** is not on the critical path — but the **managed certs must be
ACTIVE** (NS delegation or per-host CNAMEs in place). The internal test runs
from an in-cluster pod and resolves hostnames **by name through the private
zone** — no IP overrides — trusting the CAS root via the `cas-root` ConfigMap.
The drain/zone-failover cases **cordon and drain nodes** (uncordoned afterward,
including by `--cleanup` after a failed run); the backup→restore case runs
**last** because the restore bounces the workload namespaces.
